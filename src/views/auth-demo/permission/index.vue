<template>
  <div class="h-full">
    <n-card title="权限切换" class="h-full shadow-sm rounded-16px">
      ...
      <div>
        <n-button v-permission="'super'" class="mr-12px">1-super可见</n-button>
        <n-button v-permission="'admin'" class="mr-12px">1-admin可见</n-button>
        <n-button v-permission="['admin', 'user']">1-admin和user可见</n-button>
      </div>
      <div class="py-12px">
        <n-gradient-text type="primary" :size="20">权限函数 hasPermission</n-gradient-text>
      </div>
      <n-space>
        <n-button v-if="hasPermission('super')">2-super可见</n-button>
        <n-button v-if="hasPermission('admin')">2-admin可见</n-button>
        <n-button v-if="hasPermission(['admin', 'user'])">2-admin和user可见</n-button>
      </n-space>
    </n-card>
  </div>
</template>
